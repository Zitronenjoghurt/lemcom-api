...
use crate::api::utils::time_operations::{timestamp_now_nanos, nanos_to_date};
...
impl User {
    ...
    pub fn public_information(&self) -> UserPublicInformation {
        let joined_date = if self.join_date_public {nanos_to_date(self.created_stamp)} else {String::new()};
        let last_online_date = if self.online_date_public {nanos_to_date(self.last_access_stamp)} else {String::new()};
...
pub async fn find_user_by_name(collection: &Collection<User>, name: &str) -> mongodb::error::Result<Option<User>> {
